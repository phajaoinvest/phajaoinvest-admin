...
interface StocksState {
  ...
  updateStock: (id: string, data: Parameters<typeof stocksApi.update>[1]) => Promise<Stock>
  deleteStock: (id: string) => Promise<void>
  setStocksFilters: (filters: Partial<StockFilters>) => void
  
  // Stock picks actions
  fetchStockPicks: (params?: PaginationParams & StockPickFilters) => Promise<void>
  fetchPendingApprovals: () => Promise<void>
  createStockPick: (data: Parameters<typeof stockPicksAdminApi.create>[0]) => Promise<StockPick>
  updateStockPick: (id: string, data: Parameters<typeof stockPicksAdminApi.update>[1]) => Promise<StockPick>
...
export const useStocksStore = create<StocksState>((set, get) => ({
  ...
  fetchStocks: async (params) => {
    ...
    try {
      ...
      const message = (error as { message?: string })?.message || 'Failed to fetch stocks'
  ...
  fetchStockById: async (id) => {
    ...
    try {
      ...
      const message = (error as { message?: string })?.message || 'Failed to fetch stock'
      set({ error: message, isLoading: false })
      return null
    }
  },
  ...
  createStock: async (data) => {
    ...
    try {
      ...
      const message = (error as { message?: string })?.message || 'Failed to create stock'
  ...
  updateStock: async (id, data) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        currentStock: state.currentStock?.id === id ? response.data : state.currentStock,
        isUpdating: false,
      }))
      return response.data
    } catch (error) {
      const message = (error as { message?: string })?.message || 'Failed to update stock'
  ...
  deleteStock: async (id) => {
    ...
    try {
      ...
      const message = (error as { message?: string })?.message || 'Failed to delete stock'
  ...
  fetchStockPicks: async (params) => {
    ...
    try {
      ...
      const response = await stockPicksAdminApi.getAll({ ...stockPicksFilters, ...params })
      ...
      const message = (error as { message?: string })?.message || 'Failed to fetch stock picks'
  ...
  fetchPendingApprovals: async () => {
    ...
    try {
      ...
      const message = (error as { message?: string })?.message || 'Failed to fetch pending approvals'
  ...
  createStockPick: async (data) => {
    ...
    try {
      ...
      const message = (error as { message?: string })?.message || 'Failed to create stock pick'
      set({ error: message, isCreating: false })
      throw error
    }
  },
  ...
  updateStockPick: async (id, data) => {
    ...
    try {
      ...
      set((state) => ({
        stockPicks: state.stockPicks.map((sp) => (sp.id === id ? response.data : sp)),
        currentStockPick: state.currentStockPick?.id === id ? response.data : state.currentStockPick,
        isUpdating: false,
      }))
      return response.data
    } catch (error) {
      const message = (error as { message?: string })?.message || 'Failed to update stock pick'
      set({ error: message, isUpdating: false })
      throw error
    }
  },
  ...
  deleteStockPick: async (id) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        currentStockPick: state.currentStockPick?.id === id ? null : state.currentStockPick,
        isDeleting: false,
      }))
    } catch (error) {
      const message = (error as { message?: string })?.message || 'Failed to delete stock pick'
      set({ error: message, isDeleting: false })
      throw error
    }
  },
  ...
  approveStockPick: async (id) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        stockPicks: state.stockPicks.map((sp): StockPick => 
          sp.id === id ? { ...sp, status: StockPickStatus.APPROVED } : sp
        ),
        isUpdating: false,
      }))
    } catch (error) {
      const message = (error as { message?: string })?.message || 'Failed to approve stock pick'
      set({ error: message, isUpdating: false })
      throw error
    }
  },
  ...
  rejectStockPick: async (id, reason) => {
    ...
    try {
      ...
      const message = (error as { message?: string })?.message || 'Failed to reject stock pick'
      set({ error: message, isUpdating: false })
      throw error
    }
  },
